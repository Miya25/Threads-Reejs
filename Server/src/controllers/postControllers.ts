...
export const createPost: RequestHandler = async (
  ...
  if (!content && !req.file?.path) {
    throw createHttpError(
      400,
      "Bad request, post should have a content or image",
    );
  }
...
export const getSinglePost: RequestHandler<PostParam> = async (
  req,
  res,
  next,
) => {
...
export const getPosts: RequestHandler = async (req, res, next) => {
  try {
    ...
    const posts = await PostModel.find({ parent: null })
      .sort({ createdAt: -1 })
      .populate([
        { path: "creator" },
        { path: "liked_by" },
...
// get user's post
export const getUserPosts: RequestHandler<GetUserPostsOrRepliesParam> = async (
  req,
  res,
  next,
) => {
  ...
  try {
    ...
    const userPosts = await PostModel.find({ creator: userId, parent: null })
      .sort({ createdAt: -1 })
      .populate("liked_by")
      .populate("creator")
      .exec();
...
// update post
// NEEDS AUTHENTICATED USER IN ORDER TO ACCESS THIS
export const updatePost: RequestHandler<PostParam> = async (
  req: CustomRequest,
  res,
  next,
) => {
  ...
  try {
    ...
    if (authenticatedUserId) {
      if (!post.creator.equals(authenticatedUserId)) {
        throw createHttpError(
          403,
          "Forbidden, unauthorized to delete the post",
        );
      }
    }
...
// delete post
// NEEDS AUTHENTICATED USER IN ORDER TO ACCESS THIS
export const deletePost: RequestHandler<PostParam> = async (
  req: CustomRequest,
  res,
  next,
) => {
  ...
  try {
    ...
    if (post.parent) {
      ...
      parentPost.children = parentPost?.children.filter(
        (childPost) => !childPost.equals(postId),
      );
...
// like post
// NEEDS AUTHENTICATED USER IN ORDER TO ACCESS THIS
export const likePost: RequestHandler<PostParam> = async (
  req: CustomRequest,
  res,
  next,
) => {
  ...
  try {
    ...
    const didLike = post.liked_by.some((user) =>
      user.equals(new mongoose.Types.ObjectId(authenticatedUserId)),
    );
...
// NEEDS AUTHENTICATED USER IN ORDER TO ACCESS THIS
export const unlikePost: RequestHandler<PostParam> = async (
  req: CustomRequest,
  res,
  next,
) => {
  ...
  try {
    ...
    post.liked_by = post.liked_by.filter(
      (user) => !user.equals(new mongoose.Types.ObjectId(authenticatedUserId)),
    );
...
export const createPostReply: RequestHandler<PostParam> = async (
  req: CustomRequest,
  res,
  next,
) => {
  ...
  try {
    ...
    if (!content && !req.file?.path) {
      throw createHttpError(
        400,
        "Bad request, reply should have a content or image",
      );
    }
...
export const getPostReplies: RequestHandler<PostParam> = async (
  req,
  res,
  next,
) => {
  ...
  try {
    const postReplies = await PostModel.find({ parent: parentId })
      .sort({ createdAt: -1 })
      .populate([
        { path: "creator" },
        { path: "liked_by" },
...
export const getUserReplies: RequestHandler<
  GetUserPostsOrRepliesParam
> = async (req, res, next) => {
  const userId = req.params.userId;
  ...
  try {
    ...
    const userReplies = await PostModel.find({
      creator: userId,
      parent: { $ne: null },
    })
      .sort({ createdAt: -1 })
