...
export const getAuthenticatedUserInfo: RequestHandler = async (
  req: CustomRequest,
  res,
  next,
) => {
...
export const getUserInfo: RequestHandler<GetUserInfoParam> = async (
  req,
  res,
  next,
) => {
...
export const updateUserInfo: RequestHandler = async (
  ...
  try {
    ...
    user.displayed_picture = {
      ...
      cloudinary_id:
        image?.cloudinary_id || user.displayed_picture?.cloudinary_id,
    };
...
export const followUser: RequestHandler<FollowAndUnfollowParam> = async (
  req: CustomRequest,
  res,
  next,
) => {
  ...
  try {
    ...
    const authenticatedFollower = await UserModel.findById(
    ...
    const isFollowing = userToFollow.followers.some((followerId) =>
      followerId.equals(new mongoose.Types.ObjectId(authenticatedFollowerId)),
    );
    ...
    userToFollow.followers.push(
      new mongoose.Types.ObjectId(authenticatedFollowerId),
    );
    authenticatedFollower.following.push(
...
export const unfollowUser: RequestHandler<FollowAndUnfollowParam> = async (
  req: CustomRequest,
  res,
  next,
) => {
  ...
  try {
    ...
    const isFollowing = userToUnfollow.followers.some((followerId) =>
      followerId.equals(new mongoose.Types.ObjectId(authenticatedFollowerId)),
    );

    if (!isFollowing)
      throw createHttpError(400, "You already unfollowed this user");
    ...
    authenticatedFollower.following = authenticatedFollower.following.filter(
      (followerId) =>
        !followerId.equals(new mongoose.Types.ObjectId(userToUnfollowId)),
    );
...
export const getUserPostsAndReposts: RequestHandler<
...
> = async (req, res, next) => {
  ...
  try {
    ...
    const userReposts = await RepostModel.find({ repost_creator: userId })
      .populate({
        path: "post",
        select: "-updatedAt",
        populate: [
...
export const searchUsers: RequestHandler<SearchUserQuery> = async (
  req,
  res,
  next,
) => {
  ...
  try {
    if (!user) {
      throw createHttpError(
        403,
        "Forbidden, unauthorized to get the random users",
      );
    }
...
export const getUsers: RequestHandler = async (
  ...
  try {
    ...
    const populatedUsers = await UserModel.find({
      username: { $ne: authenticatedUser.username },
    })
      .populate("followers")
      .exec();
